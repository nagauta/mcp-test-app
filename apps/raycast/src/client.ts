  ...
  nodes: {
    ...
    dbg: {
      agent: async (inputs: any) => {
        console.log(`xxxxxxxxxxxxxxxx: ${JSON.stringify(inputs.tool2)}`);
        console.log(`xxxxxxxxxxxxxxxx: ${JSON.stringify(inputs.tool3)}`);
        return;
      },
...
if(result) {
  console.log(`resultxxx: ${JSON.stringify(result.messages)}`);
